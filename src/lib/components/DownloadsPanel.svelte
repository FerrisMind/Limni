...
{#if browserState.showDownloads}
  <div class="downloads-panel">
    <div class="panel-header">
      <h3>Загрузки</h3>
      <button class="close-btn" onclick={() => browserState.showDownloads = false}>
        <i class="ph ph-x"></i>
      </button>
    </div>
        ...
        {#each downloads as download}
          <div class="download-item">
            <div class="download-info">
              ...
              <div class="file-details">
                ...
                <div class="file-meta">
                  ...
                  <span class="file-status" class:completed={download.status === 'completed'} 
            ...
            <div class="download-actions">
              {#if download.status === 'completed'}
                <button class="action-btn" onclick={() => openDownload(download.id)} title="Открыть">
                  <i class="ph ph-folder-open"></i>
                </button>
              {:else if download.status === 'downloading'}
                <button class="action-btn" onclick={() => pauseDownload(download.id)} title="Пауза">
                  <i class="ph ph-pause"></i>
                </button>
              {:else if download.status === 'failed'}
                <button class="action-btn" onclick={() => retryDownload(download.id)} title="Повторить">
                  <i class="ph ph-arrow-clockwise"></i>
                </button>
              {/if}
              <button class="action-btn danger" onclick={() => removeDownload(download.id)} title="Удалить">
