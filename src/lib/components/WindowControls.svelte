...
<div class="window-controls" onmousedown={handleDragStart} role="toolbar" tabindex="0">
  ...
  <button class="new-tab-btn" onclick={handleNewTab} title="Новая вкладка" aria-label="Новая вкладка">
  ...
  <button class="control-btn minimize" onclick={minimizeWindow} title="Свернуть" aria-label="Свернуть окно">
    <i class="ph ph-minus"></i>
  </button>
  
  <button class="control-btn maximize" onclick={toggleMaximize} title={windowState?.isMaximized ? "Восстановить" : "Развернуть"} aria-label={windowState?.isMaximized ? "Восстановить окно" : "Развернуть окно"}>
    {#if windowState?.isMaximized}
  ...
  <button class="control-btn close" onclick={closeWindow} title="Закрыть" aria-label="Закрыть окно">
    <i class="ph ph-x"></i>
  </button>
</div>
...
<style>
  .window-controls {
    ...
    padding: var(--container-padding); /* Делаем одинаковые отступы справа, сверху и снизу (все по 6px) */
  ...
  .new-tab-btn:hover {
    background: rgba(255, 255, 255, 0.2); /* Делаем фон ховера более заметным */
