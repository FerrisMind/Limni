<script lang="ts">
  import { bookmarks, removeBookmark, addBookmark, getActiveTab, updateTabUrl } from '../stores/browser.svelte.js';
  ...
  <div class="bookmarks-panel">
    <div class="panel-header">
      ...
      <div class="header-actions">
        <button class="action-btn" onclick={addCurrentPageToBookmarks} title="Добавить текущую страницу">
          <i class="ph ph-star"></i>
        </button>
        <button class="action-btn close-btn" onclick={closePanel} title="Закрыть">
          <i class="ph ph-x"></i>
        </button>
      </div>
    </div>
    ...
    <div class="bookmarks-content">
      {#if bookmarks.length === 0}
        <div class="empty-state">
          ...
          <p class="empty-hint">Нажмите <i class="ph ph-star"></i> чтобы добавить текущую страницу в закладки</p>
        </div>
      {:else}
        <div class="bookmarks-list">
          {#each bookmarks as bookmark (bookmark.id)}
            <button class="bookmark-item" onclick={() => openBookmark(bookmark.url, bookmark.title)}>
...
<style>
  .bookmarks-panel {
    position: fixed;
    top: calc(var(--tabbar-height) + var(--toolbar-height)); /* Правильная высота: 44px + 45px = 89px */
    left: 0;
    width: 352px;
