<script lang="ts">
  ...
  function groupHistoryByDate(): Map<string, HistoryEntry[]> {
  ...
  let filteredHistory = $derived(history.filter(entry => 
  ...
  <div class="history-panel">
    <div class="panel-header">
      ...
      <div class="header-actions">
        <button class="action-btn" onclick={clearAllHistory} title="Очистить историю">
          ...
          {#each [...groupedHistory().entries()] as [dateGroup, entries] (dateGroup)}
            <div class="history-group">
              <div class="group-header">
                <h4>{dateGroup}</h4>
                <span class="group-count">{entries.length} {entries.length === 1 ? 'посещение' : 'посещений'}</span>
              </div>
