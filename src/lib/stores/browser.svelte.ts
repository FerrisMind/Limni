...
listen<{tabId: string, title: string}>('webview-title-changed', (event) => {
  ...
  console.log('🔍 Title generic flags:', { newIsGeneric, currentIsGeneric });

  // Skip update if the incoming title is generic while the current one is specific
  if (!currentIsLoading && newIsGeneric && !currentIsGeneric) {
    console.log('🔍 Skipping title update: new title is generic but current is specific');
    return;
  }
